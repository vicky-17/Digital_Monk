...
object NotificationHelper {
    ...
    fun buildGuardianForegroundNotification(context: Context): Notification {
        return NotificationCompat.Builder(context, Constants.CHANNEL_GUARDIAN)
            .setSmallIcon(com.curbme.app.R.drawable.ic_launcher_foreground)
            .setContentTitle("CurbMe Protection Active")
    ...
    fun buildUsageNotification(
        ...
        if (bypassRemainingMs > 0) {
            title = "⏳ Regain active: ${com.curbme.app.core.utils.TimeUtils.formatDuration(bypassRemainingMs)} left"
            text = "Total Usage: ${com.curbme.app.core.utils.TimeUtils.formatDuration(totalUsageMs)}"
        } else if (appName != null) {
            // App Open Design
            title = "$appName: ${com.curbme.app.core.utils.TimeUtils.formatDuration(appUsageMs)}"
            text = "Total Usage: ${com.curbme.app.core.utils.TimeUtils.formatDuration(totalUsageMs)}"
        } else {
            // Home Screen / System Design
            title = "Total Usage: ${com.curbme.app.core.utils.TimeUtils.formatDuration(totalUsageMs)}"
            text = "CurbMe protection is active."
        }

        val openIntent = android.app.PendingIntent.getActivity(
            context, 0, android.content.Intent(context, com.curbme.app.ui.MainActivity::class.java),
        ...
        // ── Live Update (Android 14+) ─────────────────────────────────────────
        if (android.os.Build.VERSION.SDK_INT >= 34) {
            builder.setForegroundServiceBehavior(NotificationCompat.FOREGROUND_SERVICE_IMMEDIATE)
            
            // Add the extra to request promotion to a status bar chip
            builder.addExtras(android.os.Bundle().apply {
                putBoolean("android.app.extra.REQUEST_PROMOTED_ONGOING", true)
            })

            // On very new versions, we could use setShortCriticalText if available
            // but for now, the title and chronometer/when will drive the chip.
        }
