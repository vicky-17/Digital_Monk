...
object PersistenceManager {
    ...
    val autostartInstructions: String
        get() {
            when (detectOem()) {
                OemType.XIAOMI -> return "Security → Manage Apps → CurbMe → Autostart → Enable"
                OemType.OPPO -> return "Phone Manager → Privacy Permissions → Startup Manager → CurbMe → Allow"
                OemType.VIVO -> return "iManager → App Manager → Autostart → CurbMe → Enable"
                OemType.HUAWEI -> return "System Manager → App Launch → CurbMe → Manage manually → Enable all"
                OemType.SAMSUNG -> return "Device Care → Battery → Background Usage Limits → Never Sleeping Apps → Add CurbMe"
                OemType.ONEPLUS -> return "Settings → Battery → Battery Optimization → CurbMe → Don't Optimize"
                OemType.ASUS -> return "Mobile Manager → Autostart → CurbMe → Enable"
                else -> return "Settings → Battery → Battery Optimization → CurbMe → Don't Optimize"
            }
        }
